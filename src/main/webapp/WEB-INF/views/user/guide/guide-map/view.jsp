	...
	*, *:before, *:after {
	...
	.stats-counter {
		background: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)),
			url("/dd/resources/files/guide/guide-map.jpg") center center;
		background-size: cover;
	...
	#webtong_tab_type04 li a:before, .webtong_tab_type04 li a:before {
		content: '';
		display: block;
		position: absolute;
		left: 0;
	...
	#webtong_tab_type04 li.on a:before, .webtong_tab_type04 li:hover a:before
		{
		width: 100%
	}
	
	#webtong_tab_type04 li.on a, .webtong_tab_type04 li:hover a {
		color: #fd7c82;
		font-weight: 600
	}
	...
	@media screen and (max-width: 780px) {
		...
		#webtong_tab_type04 li:nth-child(n+4) {
			margin-top: -1px
		}
		#webtong_tab_type04:before {
			content: '';
		...
		<div class="gy-4" style="justify-content: center; width: 100%;">
			<div class="col-lg-3 col-md-6" style="width: 100%;">
				<div class="stats-item text-center w-100 h-100">
					<div id="title"
						style="font-size: 48px; display: block; color: #fff; font-weight: 700;">가이드맵</div>
					<p style="font-size: 20px;">
						.D_D. 랜드를 한번에 보기! <i class="fa-regular fa-face-laugh-squint"
							style="color: #000;"></i>
					</p>
				</div>
			</div>
		...
		<div class="tab-content" data-aos-delay="900">
			<div class="tab-pane fade active show" id="menu-starters"
				...
				<div class="form-group">
					<div id="contents">
						<ul class="tab tab_red" id="webtong_tab_type04"
							style="width: 1300px; padding: 0; margin-top: 50px;">
					...
					<div id="tab01" style="display: none;">
						<div id="main_box">
							<p
								style="text-align: center; font-weight: bold; margin-top: 20px; padding-bottom: 20px;">현재
								등록되어 있는 어트랙션입니다.</p>
							<hr>
							<div
...
<script>
	    ...
	    const map = new kakao.maps.Map(container, options); //지도 생성 및 객체 리턴
	  	...
	  	// 위에까지는 지도 생성 후, 해당 리스트에 맞는 마커 표시하기
	  ...
	  contentBoxes.forEach((box, index) => {
	    box.addEventListener('click', function (event) {
	      ...
	      contentBoxes.forEach((otherBox) => {
	        if (otherBox !== this) {
	          otherBox.style.backgroundColor = 'white'; // 선택되지 않은 상자의 배경색을 평소 흰색으로 변경
	        }
	      });
	      this.style.backgroundColor = 'gold'; // 클릭된 상자의 배경색을 gold로 변경
	    ...
	    box.addEventListener('mouseover', function (event) {
	      if (this.style.backgroundColor !== 'gold') {
	        this.style.backgroundColor = '#CCC'; // hover 시 선택되지 않은 상자의 배경색을 #CCC로 변경
	      }
	    });

	    box.addEventListener('mouseout', function (event) {
	      if (this.style.backgroundColor !== 'gold') {
	        this.style.backgroundColor = 'white'; // hover 종료 시 선택되지 않은 상자의 배경색을 평소 흰색으로 변경
	      }
	    });
	  });
	});
