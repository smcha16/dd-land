						...
						<div class="card">
                			<div class="card-body">
                  				...
                  				<nav class="d-flex justify-content-end">
                    				<ol class="breadcrumb">
                      					<li class="breadcrumb-item"><a href="/dd/admin/activity/photozone/add.do">추가</a></li>
                  				...
                  				<form id="del-form" method="POST" action="/dd/admin/activity/photozone/del.do">
	                  				<table class="table">
	                    				<thead>
	                      					<tr>
	                        					<th></th>
									...
									<ul class="pagination justify-content-center">
										<c:forEach begin="1" end="${map.totalPages}" varStatus="pageStatus">
											<c:choose>
												...
												<c:otherwise>
													<li class="page-item"><a class="page-link" href="/dd/admin/activity/photozone/view.do?page=${pageStatus.index}">${pageStatus.index}</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>
									</ul>
...
<!-- admin > activity > photozone > view JavaScript -->
<!-- Kakao Map Open API -->
...
<script>
	...
	function edit() {
		...
		if (checkedCount > 1 || checkedCount < 1) {
			alert('1개의 포토존을 선택 후, 수정 버튼을 눌러주세요.');
		} else {

			const seq = $('input[type="checkbox"]:checked').val();
			
			location.href='/dd/admin/activity/photozone/edit.do?seq=' + seq;
	...
	function del() {
		...
		if (checkedCount == 0) {
			alert('1개 이상의 포토존을 선택 후, 삭제 버튼을 눌러주세요.');
		} else {
			
			if (confirm('선택한 포토존을 삭제하시겠습니까?')) {
	...
	/* 포토존 상세 모달 */
	function showModal(seq, name, time, info) {
	...
	function addModalImg(seq) {
		...
		const filterImg = img_list.filter(obj => obj.photozone_seq == seq);
		
		if (filterImg.length > 0) {
			
			filterImg.forEach(obj => {
				
				let imgSrc = '/dd/resources/files/activity/photozone/' + obj.img;
	...
	/* 포토존 위치 모달이 열릴 때 카카오맵 relayout 설정 */
	...
	<c:forEach items="${ilist}" var="dto">
		img_list.push({
			photozone_img_seq: ${dto.photozone_img_seq},
			img: `${dto.img}`,
			photozone_seq: ${dto.photozone_seq}
		});
	</c:forEach>
