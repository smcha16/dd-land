	...
	<div class="pagetitle">
		...
		<nav class="d-flex justify-content-end">
      		<ol class="breadcrumb">
          		...
          		<li class="breadcrumb-item active"><a href="/dd/admin/shop/restaurant/view.do">취소</a></li>
      		</ol>
		</nav>
  	</div>
  	...
  	<section class="section">
    	<div class="row">
     		 <div class="col-lg-12">
        		...
        		<div class="card">
          			<div class="card-body">
            			<h5 class="card-title">레스토랑 정보 입력</h5>

            			<form method="POST" action="/dd/admin/shop/restaurant/addok.do" enctype="multipart/form-data">
            			
              				<div class="row mb-3">
                				<label for="name" class="col-sm-2 col-form-label required">레스토랑명</label>
                				<div class="col-sm-10">
                  					<input type="text" id="name" name="name" class="form-control" placeholder="레스토랑명을 입력해주세요." required>
              				...
              				<div class="row mb-3">
                				<label for="info" class="col-sm-2 col-form-label required" >메뉴</label>
                				<div class="col-sm-10">
                  					<textarea id="info" name="menu" class="form-control" style="height: 100px" placeholder="레스토랑의 대표 메뉴들을 입력해주세요." required></textarea>
                				</div>
              				</div>
              				
              				<div class="row mb-3">
                				<label for="time" class="col-sm-2 col-form-label required" >운영 시간</label>
                				<div class="col-sm-10">
                					<input type="text" id="time" name="time" class="form-control" placeholder="운영 시간(HH:MM - HH:MM)을 입력해주세요." required>
                					<div style="height: 30px;">
              				...
              				<div class="row mb-3">
                				<label for="capacity" class="col-sm-2 col-form-label required">수용 인원</label>
                				<div class="col-sm-10">
                  					<input type="number" id="capacity" name="capacity" class="form-control" placeholder="수용 인원(숫자)을 입력해주세요." required>
                				</div>
              				</div>

              				<div class="row mb-3">
                				<label for="tel" class="col-sm-2 col-form-label required">전화번호</label>
                				<div class="col-sm-10">
                  					<input type="text" id="tel" name="tel" class="form-control" placeholder="레스토랑 전화번호(070-XXXX-XXXX)를 입력해주세요." required>
                  					<div style="height: 30px;">
                  						<div class="check-tel-validation"></div>
                  					</div>
                				</div>
              				</div>
...
<script>
	...
	function submit() {
		...
		/* if (!$('input[name="name"]').val().trim() || !$('textarea[name="menu"]').val().trim()
	...
	$('input[name="time"]').keyup(function() {
		...
		if (pattern.test(time)) {
		    ...
		    $('.check-time-validation').text('올바른 형식(HH:MM - HH:MM)으로 입력해주세요.');
	...
	$('input[name="tel"]').keyup(function() {
		...
		if (telPattern.test(tel)) {
		    $('.check-tel-validation').text('');
		    $('.check-tel-validation').css('color', '#212529');
		    $('.check-tel-validation').data('type', 'y');
		    
		} else {
		    $('.check-tel-validation').text('올바른 형식(070-XXXX-XXXX)으로 입력해주세요.');
		    $('.check-tel-validation').css('color', '#dc3545');
		    $('.check-tel-validation').data('type', 'n');
		}
	...
	$('input[name="name"]').keyup(function() {
        ...
        if ($('input[name="name"]').val().trim()) {
			...
			$.ajax({
				type: 'POST',
				url: '/dd/admin/shop/restaurant/name',
				headers: {'content-Type': 'application/json'},
				data: JSON.stringify(obj),
				dataType: 'json',
				success: function(result) {
					if (result == 0) {
						$('.check-name-duplication').text('사용 가능한 레스토랑명입니다.');
						$('.check-name-duplication').css('color', '#212529');
						$('.check-name-duplication').data('type', 'y');
						console.log("check-name-duplication: " + $('.check-name-duplication').data('type'));
						
					} else {
						$('.check-name-duplication').text('중복된 레스토랑명입니다. 다른 레스토랑명을 입력해주세요.');
