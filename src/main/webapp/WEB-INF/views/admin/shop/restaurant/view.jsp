						...
						<div class="card">
                			<div class="card-body">
                  				...
                  				<nav class="d-flex justify-content-end">
                    				<ol class="breadcrumb">
                      					<li class="breadcrumb-item"><a href="/dd/admin/shop/restaurant/add.do">추가</a></li>
                  				...
                  				<form id="del-form" method="POST" action="/dd/admin/shop/restaurant/del.do">
	                  				<table class="table">
	                    				<thead>
	                      					<tr>
	                        					<th></th>
	                    				...
	                    				<tbody>
	                    					<c:forEach items="${list}" var="dto" varStatus="status">
		                      					<tr>
		                        					<td><input type="checkbox" name="restaurant_seq" value="${dto.restaurant_seq}"></td>
									...
									<ul class="pagination justify-content-center">
										<c:forEach begin="1" end="${map.totalPages}" varStatus="pageStatus">
											<c:choose>
												...
												<c:otherwise>
													<li class="page-item"><a class="page-link" href="/dd/admin/shop/restaurant/view.do?page=${pageStatus.index}">${pageStatus.index}</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>
									</ul>
...
<script>
	...
	function edit() {
		...
		if (checkedCount > 1 || checkedCount < 1) {
			...
			location.href='/dd/admin/shop/restaurant/edit.do?seq=' + seq;
	...
	function addModalImg(seq) {
		...
		const filterImg = img_list.filter(obj => obj.restaurant_seq == seq);
		
		if (filterImg.length > 0) {
			
			filterImg.forEach(obj => {
				
				let imgSrc = '/dd/resources/files/restaurant/' + obj.img;
	...
	<c:forEach items="${ilist}" var="dto">
		img_list.push({
			restaurant_img_seq: ${dto.restaurant_img_seq},
			img: `${dto.img}`,
			restaurant_seq: ${dto.restaurant_seq}
		});
	</c:forEach>
